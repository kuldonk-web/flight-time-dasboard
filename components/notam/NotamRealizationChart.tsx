...
import { computeNotamExpiryTrend } from '@/utils/notamStats';
...
export function NotamRealizationChart({ notams, activeCount, expiredCount }: Props) {
  const trend = computeNotamExpiryTrend(notams);

  return (
    <div className="flex flex-col gap-3 rounded-sm border border-border bg-surface-raised p-4">
      <h3 className="text-xs font-semibold uppercase tracking-wide text-text-secondary">Realisasi NOTAM</h3>
      <div className="grid grid-cols-3 gap-2 sm:max-w-md">
        <SummaryCard label="Total NOTAM" value={notams.length} />
        <SummaryCard label="Aktif" value={activeCount} />
        <SummaryCard label="Kadaluarsa" value={expiredCount} />
      </div>
      {trend.length > 0 && (
        <div className="h-48 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={trend}>
              ...
              <Bar dataKey="count" name="NOTAM Tutup" fill="#2563eb" radius={[4, 4, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      )}
